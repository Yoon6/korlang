...
auto Function::print(int depth)->void {
    indent(depth); cout << "FUNCTION " << converter.to_bytes(name) << ": " << endl;
    if (parameters.size()) {
        indent(depth + 1); cout << "PARAMETERS:";
        for (auto& name : parameters)
            cout << converter.to_bytes(name) << " ";
        cout << endl;
    }
...
auto Variable::print(int depth)->void {
    indent(depth); cout << "VAR " << converter.to_bytes(name) << ":" << endl;
    expression->print(depth + 1);
}

auto Print::print(int depth)->void {
    indent(depth); cout << "PRINT:" << endl;
...
auto Relational::print(int depth)->void {
    indent(depth); cout << converter.to_bytes(toString(kind)) << ":" << endl;
    indent(depth + 1); cout << "LHS:" << endl;
    lhs->print(depth + 2);
    indent(depth + 1); cout << "RHS:" << endl;
    rhs->print(depth + 2);
...
auto StringLiteral::print(int depth)->void {
    indent(depth); cout << "\"" << converter.to_bytes(value) << "\"" << endl;
}
...
auto GetVariable::print(int depth)->void {
    indent(depth); cout << "GET_VARIABLE: " << converter.to_bytes(name) << endl;
}

auto SetVariable::print(int depth)->void {
    indent(depth); cout << "SET_VARIABLE: " << converter.to_bytes(name) << endl;
    value->print(depth + 1);
}
